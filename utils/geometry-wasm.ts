...
export function splitCurve(
  ...
  return [p0, { x: (p0.x + p1.x) / 2, y: (p0.y + p1.y) / 2 }, { x: (q0.x + p2.x) / 2, y: (q0.y + p2.y) / 2 }, q0, q0, { x: (q0.x + p2.x) / 2, y: (q0.y + p2.y) / 2 }, { x: (p2.x + p3.x) / 2, y: (p2.y + p3.y) / 2 }, p3];
}
...
export function curveLineIntersect(p0: Point, p1: Point, p2: Point, p3: Point, a: Point, b: Point, segments = 24): Point[] {
  if (wasmMod) {
    const r = wasmMod.curve_line_intersect(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, a.x, a.y, b.x, b.y, segments);
    const pts: Point[] = [];
...
export function calculateSnaps(
  ...
  if (!isNaN(snapResult[0]) && offset + 3 <= snapResult.length) {
    result.lines.push({ type: 'vertical', value: snapResult[offset + 1], origin: snapResult[offset + 2], extent: snapResult[offset + 3] });
    offset += 4;
  }
  if (!isNaN(snapResult[1]) && offset + 3 <= snapResult.length) {
    result.lines.push({ type: 'horizontal', value: snapResult[offset + 1], origin: snapResult[offset + 2], extent: snapResult[offset + 3] });
  }
