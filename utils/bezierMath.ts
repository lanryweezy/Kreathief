...
export class BezierMath {
     ...
     */
    static getPointOnCubic(p0: VectorPoint, p1: VectorPoint, p2: VectorPoint, p3: VectorPoint, t: number): { x: number, y: number } {
        const mt = 1 - t;
        const mt2 = mt * mt;
        const mt3 = mt2 * mt;
        const t2 = t * t;
     ...
     * Returns generic point objects {x, y}
     */
    static splitCubic(p0: VectorPoint, p1: VectorPoint, p2: VectorPoint, p3: VectorPoint, t: number) {
     ...
     * Uses numerical approximation (scanning + binary search refinement)
     */
    static getClosestT(p0: VectorPoint, p1: VectorPoint, p2: VectorPoint, p3: VectorPoint, point: { x: number, y: number }): { t: number, dist: number } {
        const SCANS = 20;
        let bestT = 0;
        let minMsg = Infinity;
