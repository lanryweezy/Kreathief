...
import { MatrixMath as MatrixUtils } from './matrixMath';
...
export class GeometryOracle {
    ...
    static measureText(layer: TextLayer): { width: number; height: number; ascent: number; descent: number } {
        ...
        if (!ctx) return { width: layer.width, height: layer.fontSize, ascent: layer.fontSize, descent: 0 };
    ...
    static measurePath(pathData: string) {
        ...
        if (!path) return { totalLength: 0, getPointAt: (d: number) => ({ x: 0, y: 0, angle: 0 }) };
        ...
        try {
            ...
            return { totalLength: 0, getPointAt: (d: number) => ({ x: 0, y: 0, angle: 0 }) };
        }
