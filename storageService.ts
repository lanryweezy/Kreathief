...
class StorageService {
  ...
  // S-22: Per-user export rate limiting (20/day)
  private exportCounts = new Map<string, { count: number; resetTime: number }>();
  ...
  async saveProject(project: Project): Promise<void> {
    ...
    // S-12: Per-user storage quota enforcement (10 projects for free tier)
    ...
    // Check user plan from Supabase profile for pro users
    let isPro = false;
    ...
    if (isNewProject && existingProjects.length >= MAX_FREE_PROJECTS && !isPro && !isDev) {
  ...
  // ===== S-22: Export Rate Limiting =====

  /**
   * Check if user has exceeded daily export limit (20/day).
   * Returns true if the export is allowed.
   */
  checkExportRateLimit(userId: string): boolean {
    const now = Date.now();
    const DAY_MS = 24 * 60 * 60 * 1000;
    const MAX_EXPORTS_PER_DAY = 20;

    const entry = this.exportCounts.get(userId);
    if (!entry || now > entry.resetTime) {
      this.exportCounts.set(userId, { count: 1, resetTime: now + DAY_MS });
      return true;
    }
    if (entry.count >= MAX_EXPORTS_PER_DAY) return false;
    entry.count++;
    return true;
  }
