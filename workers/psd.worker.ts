    ...
    const processLayer = async (psdLayer: PsdLayer, parentId?: string) => {
        ...
        if (psdLayer.text) {
            ...
            const color = fillColor ?
                `rgba(${fillColor.r ?? 0}, ${fillColor.g ?? 0}, ${fillColor.b ?? 0}, ${fillColor.a ?? 1})` : '#000000';
...
async function exportLayersToPsd(width: number, height: number, layers: WorkerLayer[]): Promise<Blob> {
    ...
    const convertLayer = async (layer: WorkerLayer): Promise<PsdLayer | null> => {
        ...
        const layerHeight = isText
            ? (layer.fontSize || 24) * 1.2
            : layer.height || 0;
        ...
        if (layer.shadow) {
            ...
            const color = match ? { r: parseInt(match[1]), g: parseInt(match[2]), b: parseInt(match[3]) } : { r: 0, g: 0, b: 0 };
