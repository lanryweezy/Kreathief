...
import { Layer, ImageLayer, generateLayerId } from '../types';
...
export function useMagicActions() {
  ...
  const handleEraseObject = useCallback(
    async (layer: ImageLayer, maskPath: string) => {
      ...
      const useLocalInpainting = useStore.getState().useLocalInpainting;
      addToast?.(`Removing object using ${useLocalInpainting ? 'local LaMa' : 'Cloud AI'}...`, 'info');

      try {
        let resultUrl: string | null = null;
        if (useLocalInpainting) {
  ...
  const handleMagicExtract = useCallback(
    async (layer: ImageLayer, maskPath: string) => {
      ...
      try {
        ...
        const mattedSrc = await new Promise<string | null>(async (resolve) => {
          try {
            const transparentImageBase64 = await heavyService.removeBackground(layer.src);
            const img = new Image();
            img.crossOrigin = 'anonymous';
            img.onload = () => {
              const canvas = document.createElement('canvas');
