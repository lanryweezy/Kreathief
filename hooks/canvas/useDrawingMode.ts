...
import { brushStrategies } from '../../services/strategies/brushStrategies';
import '../../services/strategies/coreBrushStrategies';
    ...
    (e: React.PointerEvent | React.MouseEvent) => {
      ...
      if (ctx) {
        ...
        for (const pt of smoothedPoints) {
          ...
          const strategy = brushStrategies.get(brushType as string);
          if (strategy) {
            if (strategy.apply(ctx, pressureWidth, brushOpacity, ptPressure, brushColor, brushSize, drawX, drawY)) {
              continue;
            }
          }
