...
export default async function handler(req: Request) {
  ...
  // S-16: Enforce max length on prompt-like fields to prevent abuse
  ...
  if (Array.isArray(contents)) {
    for (const part of contents) {
      if (part?.text && part.text.length > MAX_PROMPT_LENGTH) {
        return new Response(JSON.stringify({ error: 'Prompt exceeds maximum length of 2000 characters' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': origin },
        });
  ...
  // S-10: Admin role check — most routes are user-level, not admin-only.
  // If a route needs admin-only access, uncomment the check below:
