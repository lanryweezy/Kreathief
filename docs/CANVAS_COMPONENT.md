...
const effectiveLayers = useMemo(() => 
  layers.map((l: Layer) => getEffectiveLayer(l)), 
  [layers, getEffectiveLayer]
);

// Memoized snap line calculations
const getSnapLines = useCallback((layers) => {
  // Complex geometry calculations
}, [canvasSize, zoom]);

// Memoized handle positions
const resizeHandles = useMemo(() => 
  calculateResizeHandles(selectedBounds),
  [selectedBounds, zoom]
);
```
 ...
 */
const getLayerAtPoint = useCallback((x: number, y: number): string | null => {
 ...
 */
const enterPathEditMode = useCallback((pathId: string) => {
