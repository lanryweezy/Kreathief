...
## 2026-09-03 - Avoid observable behavior changes in logic checks

**Learning:** When removing a type cast like `as any` in a type check function (e.g. `getLayerNodeType`), adding an extra condition (e.g., `|| layer.type === 'group'`) to "improve" the check fundamentally alters the observable behavior of the code. This is treated as a logic change (a bug fix or feature tweak) rather than a pure refactoring, violating the zero-behavior-change constraint.
**Action:** When refactoring to remove type escapes, ensure the resulting logic is strictly identical to the original behavior, even if the original behavior seems incomplete. Do not add new conditions or change the strictness of checks.
