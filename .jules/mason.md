...
## 2026-09-10 - Preserving operator precedence in layout utils

**Learning:** In `utils/layoutUtils.ts`, the inline logic `l.y + (l as any).height || l.width` relied on the fact that addition `+` has a higher operator precedence than `||`. This meant it effectively executed as `(l.y + l.height) || l.width`. A naive refactoring into `l.y + getLayerHeight(l)` changed this precedence execution to `l.y + (l.height || l.width)`, which silently changed layout bounds logic.
**Action:** When refactoring inline fallback structures involving operators like `||` and mathematical operations, strictly preserve the exact output mapping.
