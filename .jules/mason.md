...
## 2026-08-27 - Properly checking interface inheritance chains before using `as any`

**Learning:** Found multiple instances of `(l as any).height` when iterating over union types like `Layer` in `utils/layoutUtils.ts`. These were used because `height` was not immediately visible on some sub-types, but actually, all sub-types of `Layer` correctly extended a common `LayerBase` interface which firmly defines `height: number;`. Therefore, there was no need for `as any` casts to appease TypeScript.
**Action:** When working with unions and encountering TypeScript property-not-found errors, trace the type definitions back through their inheritance tree (like checking `LayerBase` when `TextLayer` is part of a union) before blindly using `as any`. Often the property is already legally available on the base interface, meaning type safety can be preserved effortlessly.
