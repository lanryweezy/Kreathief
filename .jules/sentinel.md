...
## 2024-06-30 - Prevent PostgREST Filter Injection in Asset Service

**Vulnerability:** The `services/assetService.ts` file used string interpolation for the user `query` directly into a Supabase `.or()` condition (`.or(\`title.ilike.%${query}%,...\`)`). If an attacker provided a query with commas (the OR separator) and quotes, they could inject arbitrary filters.
**Learning:** This is a high-severity filter injection vulnerability, as the comma acts as a logical operator separator in PostgREST.
