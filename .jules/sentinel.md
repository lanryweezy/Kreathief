...
## 2026-09-02 - Removed Client-Side Exposure of Freepik and Replicate API Keys
**Vulnerability:** The application was directly loading `import.meta.env.VITE_FREEPIK_API_KEY` and `import.meta.env.VITE_REPLICATE_API_KEY` in `services/freepikService.ts` and `services/upscaleService.ts` respectively. This exposed backend secret keys to users via the compiled frontend bundle if developers used the `VITE_` prefix. Additionally, `upscaleService.ts` was hitting the Replicate API directly from the client.
**Learning:** In Vite, any environment variable prefixed with `VITE_` is statically injected into the client bundle at build time. Secret keys or tokens for backend services should never use this prefix and should never be loaded directly into client code. Relying on the client to communicate directly with third-party APIs using injected secrets is a severe security risk.
**Prevention:** Always implement a server-side proxy route (e.g., Edge Functions like `api/freepik.ts` and `api/replicate.ts`) to handle requests securely. The client should only interact with these proxies, and the secret keys should be safely stored in the server's environment without the `VITE_` prefix. Ensure these proxies implement proper authentication (`requireAuth`) and rate limiting to prevent abuse.
