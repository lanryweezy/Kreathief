...
## 2026-07-23 - Prevent Weak PRNG in ID Generation

**Vulnerability:** The `store/useStore.ts` file generated IDs for toasts using `Math.random().toString(36).slice(2, 6)` combined with `Date.now()`. `Math.random()` is not a cryptographically secure pseudo-random number generator (CSPRNG), making the IDs predictable and vulnerable to guessing attacks. While toasts might be a lower-risk context, this insecure pattern could easily spread to more critical entities.
**Learning:** `Math.random()` does not provide sufficient entropy and its sequence can be predicted, making it unsuitable for generating any kind of unique identifier, token, or cryptographic key in security-sensitive or multi-tenant applications.
**Prevention:** Always use `crypto.randomUUID()` (which provides 122 bits of entropy) or `crypto.getRandomValues()` for generating secure, universally unique identifiers to prevent predictable IDs and potential IDOR vulnerabilities.
