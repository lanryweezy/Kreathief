...
## 2025-02-18 - Insecure PRNG for Node IDs
**Vulnerability:** Node IDs were being generated using `Math.random().toString(36)` and `Date.now()`.
**Learning:** `Math.random()` is not cryptographically secure and predictable PRNGs can lead to ID collisions, state corruption, and React reconciliation failures when generating unique identifiers for entities like layers, shapes, or groups.
**Prevention:** Use standard, cryptographically secure APIs like `crypto.randomUUID()` (optionally combined with domain prefixes) for all client-side ID generation.
