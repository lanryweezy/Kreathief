...
## 2026-09-26 - [Remove Client-Side Exposure of Pexels API Key]
**Vulnerability:** The application was fetching `import.meta.env.VITE_PEXELS_API_KEY` and using a hardcoded fallback API key directly on the client-side within `api/pexels.ts` (which was imported by `services/assetSearch.ts`). This exposed the secret Pexels API key to users via the compiled frontend bundle.
**Learning:** Hardcoding API keys or using the `VITE_` prefix for secrets exposes them in the client bundle. Any file that relies on these and is imported into client code becomes a vulnerability vector.
**Prevention:** Ensure that external services requiring a secret API key are accessed through a secure server-side proxy route. Secrets must reside on the server (e.g., using `process.env.PEXELS_API_KEY`) and must not be prefixed with `VITE_` or exposed directly in client-imported modules.
