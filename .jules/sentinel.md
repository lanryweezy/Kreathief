...
## 2026-07-25 - Fix Insecure Pseudo-Random Number Generation for IDs

**Vulnerability:** The application was generating IDs for toast messages using a combination of `Date.now()` and `Math.random().toString(36)`. `Math.random()` is not a cryptographically secure pseudo-random number generator (CSPRNG), and `Date.now()` is highly predictable.
**Learning:** Using insecure PRNGs for unique identifiers can lead to predictability and collision attacks, especially if the IDs are later used for state management, tracking, or DOM element targeting. While toast IDs are low risk, adopting secure random generation is a fundamental security hygiene practice across the codebase.
**Prevention:** When generating unique identifiers for entities (like layers, shapes, groups, toasts), always use cryptographically secure methods like `crypto.randomUUID()` (typically combined with a domain prefix like `toast_`) instead of `Math.random()` or `Date.now()` to ensure true uniqueness and unpredictability.
