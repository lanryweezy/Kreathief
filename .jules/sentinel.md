...
## 2024-05-20 - [Unauthenticated API proxy endpoint]
**Vulnerability:** The API proxy endpoint for GetIllustration (`api/getillustration.ts`) did not have the `requireAuth` check, leaving it entirely unauthenticated. This allows anyone to abuse the endpoint to make external API calls using the server's API key, leading to quota exhaustion and financial impact.
**Learning:** All proxy API endpoints that rely on sensitive backend secrets must be secured with proper authentication to ensure only authorized users can trigger external requests on behalf of the server.
**Prevention:** If an external service requires a secret API key, implement a server-side proxy route (e.g., an Edge Function like `api/getillustration.ts`) to handle the requests securely. The client should only interact with this proxy, and the secret key should be safely stored in the server's environment without the `VITE_` prefix. Add `requireAuth(req)` check inside a try-catch block immediately after the CORS `OPTIONS` preflight request handler.
