...
## 2026-07-22 - Prevented Permissive CORS Configuration in GetIllustration API Proxy

**Vulnerability:** The API proxy endpoint `api/getillustration.ts` fell back to a wildcard CORS origin (`'*'`) if the `VITE_FRONTEND_URL` environment variable was not configured (`const origin = process.env.VITE_FRONTEND_URL || '*';`).
**Learning:** Using a wildcard (`'*'`) for `Access-Control-Allow-Origin` allows any website to make cross-origin requests to this endpoint. Because this endpoint proxies authenticated requests to a third-party service, a wildcard origin could potentially be abused if coupled with credentialed requests (though standard fetch prevents `*` with credentials, it generally exposes the API to arbitrary domains and increases attack surface). Failing securely is paramount.
**Prevention:** Never fallback to permissive configurations (`'*'`) when environmental variables defining trusted boundaries are missing. If the intended frontend URL is not configured, the server should fail securely by aborting initialization or responding with a 500 error, e.g., `if (!origin) { return new Response(JSON.stringify({ error: 'Server misconfigured' }), { status: 500 }); }`.
