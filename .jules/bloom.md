...
## 2024-11-20 - Specific error messaging for AI generation in Dashboard
**Learning:** The AI generation functions (`handleDesignGenerate` and `handleAIGenerate`) in `Dashboard.tsx` were displaying generic "Generation failed" error toasts on failure, despite the codebase having a `getAIErrorMessage` utility specifically designed to provide actionable user feedback for AI operations.
**Action:** Replaced hardcoded, generic error strings in the catch blocks of AI generation functions with `getAIErrorMessage(error)` to provide specific, actionable guidance to users.
