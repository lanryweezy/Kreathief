...
## 2026-09-17 - Typo-tolerant Design Style Search

**Learning:** Finding the right design style (e.g. "bauhaus", "brutalism") is critical to the generator, but the search relied on strict exact substring matching (`.includes()`). This caused the search to fail completely on minor user typos, degrading the user experience.
**Action:** Replaced exact substring matching with the existing `fuzzyMatch` utility from `utils/search.ts` in `searchStyles`. This closes the quality gap by adding robust typo tolerance for design style searches without altering the interface or adding new features.
