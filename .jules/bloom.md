...
## 2026-09-10 - Disregarding Automated Import Warnings for Verified Utilities
**Learning:** The automated code review process may incorrectly flag legitimate, tested project utilities (like `fuzzyMatch` from `utils/search.ts`) as "hallucinated imports" if it lacks context or fails to parse project history.
**Action:** When an import has been actively verified as functional via local read tools and passing project checks (`pnpm build`, `pnpm test run`), disregard automated code review flags about missing imports and proceed with the submission to ensure valid improvements are not blocked.
