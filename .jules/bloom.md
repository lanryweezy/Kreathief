...
## 2024-11-20 - Specific error messaging for Authentication Callbacks

**Learning:** The authentication callback (`AuthCallback.tsx`) displayed a generic "Authentication failed. Please try again." message when Supabase session retrieval failed. The underlying error object from Supabase could contain actionable information (like network issues or expired links), but it was swallowed by the generic toast.
**Action:** Replaced the hardcoded, generic error string in the `AuthCallback.tsx` error path with a dynamic message and actionable suggestion using `getErrorDetails(error)`. This closes the quality gap by ensuring users receive specific guidance when authentication fails.
