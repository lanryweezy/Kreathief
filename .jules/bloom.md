...
**Action:** Replaced exact substring matching with the existing `fuzzyMatch` utility from `utils/search.ts` to gracefully handle typos and significantly improve the search resilience without changing any component props or state structures.
\n## 2026-09-12 - Specific error messaging for Error Boundaries\n\n**Learning:** Global Error Boundaries (`ErrorBoundary`, `ErrorFallback`, `PanelErrorBoundary`) frequently displayed generic "Something went wrong" messages. While `utils/errorMessages.ts` contained a robust `getErrorDetails` utility to provide specific, actionable suggestions based on error content, it was bypassed by standard error boundaries resulting in poor user guidance on crashes.\n**Action:** Replaced hardcoded, generic error strings in `ErrorBoundary`, `ErrorFallback`, and `PanelErrorBoundary` with dynamic messages and suggestions constructed using `getErrorDetails(error)`, preserving original error messages in HTML title attributes for developer inspection. This closed the quality gap in fallback UI error reporting.\n

## 2026-09-15 - AI Design Semantic Classification Accuracy
**Learning:** Hardcoded mapping strings for semantic classifiers can drift if not checked against keyword occurrences. A prompt like 'modern penthouse luxury apartment listing' matches many more primary keywords in 'realEstate' (property, apartment, listing, penthouse, modern) than 'editorial' or 'luxury', but was failing tests because the manual test definition expected 'editorial'.
**Action:** When manually writing or fixing semantic classification tests, ensure the expected assertion precisely matches the actual output of the algorithm's weighted ruleset, rather than arbitrary human assumptions about the prompt's "vibe".
