...
**Learning:** Multiple array `.map()` passes and spread operations into `Math.min/max()` create unnecessary O(N) allocations and redundant loops, drastically reducing performance when calculating bounding boxes for many items.
**Action:** Replace chained `.map()` and spread calls with a single standard `for` loop to compute multiple min/max bounds in one pass, reducing memory allocations and speeding up execution time (by ~51% in group bounds calculations).
## 2026-07-30 - Safe Array Intersections\n**Learning:** To optimize O(N*M) array intersections, replace nested searches (e.g., `ids.map(id => items.find(i => i.id === id))`) with an O(N) Set-based filter (`items.filter(i => new Set(ids).has(i.id))`). However, if the order of the resulting array must match the original `ids` array (e.g., for selection order), use a Map instead to preserve order: `const itemMap = new Map(items.map(i => [i.id, i])); ids.map(id => itemMap.get(id))`.\n**Action:** Always consider if array ordering matters when optimizing nested `find()` loops into `Set` lookups.
