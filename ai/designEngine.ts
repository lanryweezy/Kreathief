...
export function analyzeDesign(artboards: Artboard[], layers: Layer[]): DesignAnalysis {
  ...
  return {
    score,
    suggestions,
    layout: { alignment, spacing: Math.round(spacing), balance: Math.round(balance) },
    typography: { consistency: fonts.length <= 3 ? 100 : 50, hierarchy: hasHierarchy ? 100 : 40 },
    color: { palette: colors, contrast: Math.round(contrast), harmony: colors.length <= 5 ? 90 : 50 },
  };
}
...
export function applyDesignIntent(intent: DesignIntent, artboards: Artboard[], layers: Layer[]): Layer[] {
  ...
  return getAllLayers(artboards, layers).map((layer) => {
    ...
    if (norm.includes('apple') || norm.includes('clean')) {
      ...
      if (isShapeLayer(l)) {
        l.shadow = { color: 'rgba(0,0,0,0.08)', blur: 12, offsetX: 0, offsetY: 2 };
