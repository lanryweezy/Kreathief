...
function isShapeLayer(l: Layer): l is ShapeLayer {
  return ['rectangle','circle','triangle','star','hexagon','diamond','arrow','heart','speech_bubble','ribbon','shield','banner','pentagon','octagon','plus','star_4','star_8','path'].includes(l.type);
}
...
function hexToHsl(hex: string): { h: number; s: number; l: number } {
  ...
  return { h: ((max===r?((g-b)/d+(g<b?6:0)):max===g?((b-r)/d+2):((r-g)/d+4))/6)*360, s: l>0.5?d/(2-max-min):d/(max+min), l };
}
...
function getColors(layers: Layer[]): string[] {
  return Array.from(new Set(layers.map(l => isTextLayer(l) || isShapeLayer(l) ? (l as any).color : null).filter(Boolean)));
}
...
export function analyzeDesign(artboards: Artboard[], layers: Layer[]): DesignAnalysis {
  ...
  const colors = getColors(all), fonts = getFonts(all);
  ...
  const avgSpacing = all.length > 1 ? all.reduce((a,l,i) => i===0?0:a+Math.abs(l.y-(all[i-1].y+all[i-1].height)), 0)/(all.length-1) : 0;
  const spacing = clamp(100 - Math.abs(avgSpacing-20)*2, 0, 100);
  const balance = all.length > 0 ? clamp(100 - Math.abs(all.reduce((a,l) => a+l.x+l.width/2,0)/all.length - (artboards[0]?.width||500)/2)*0.5, 0, 100) : 100;
  const contrast = all.length > 1 && colors.length > 1 ? clamp((Math.max(hexToHsl(colors[0]).l, hexToHsl(colors[1]).l)+0.05)/(Math.min(hexToHsl(colors[0]).l, hexToHsl(colors[1]).l)+0.05)*25, 0, 100) : 100;
  ...
  return { score, suggestions, layout:{alignment,spacing:Math.round(spacing),balance:Math.round(balance)}, typography:{consistency:fonts.length<=3?100:50,hierarchy:hasHierarchy?100:40}, color:{palette:colors,contrast:Math.round(contrast),harmony:colors.length<=5?90:50} };
}
...
export function applyDesignIntent(intent: DesignIntent, artboards: Artboard[], layers: Layer[]): Layer[] {
  ...
  return getAllLayers(artboards, layers).map(layer => {
    ...
    if (norm.includes('premium') || norm.includes('luxury')) {
      if (isTextLayer(l)) { l.fontFamily='Playfair Display, Georgia, serif'; l.letterSpacing=2; l.lineHeight=Math.max(l.lineHeight,1.6); }
      if (isShapeLayer(l)) l.shadow = {color:'rgba(0,0,0,0.15)',blur:20,offsetX:0,offsetY:4};
      l.opacity = clamp(l.opacity+0.05, 0, 1);
    }
    if (norm.includes('apple') || norm.includes('clean')) {
      if (isTextLayer(l)) { l.fontFamily='-apple-system, BlinkMacSystemFont, SF Pro Display, sans-serif'; l.fontWeight='400'; l.color='#1d1d1f'; }
      if (isShapeLayer(l)) { l.shadow={color:'rgba(0,0,0,0.08)',blur:12,offsetX:0,offsetY:2}; l.cornerRadius=l.cornerRadius||12; }
      l.opacity = 1;
    }
    if (norm.includes('color') || norm.includes('vibrant')) {
      const applyHsl = (c: string, sMul: number, lAdd=0) => { const h=hexToHsl(c); return `hsl(${h.h},${clamp(h.s*sMul,0,1)},${clamp(h.l+lAdd,0,1)})`; };
    ...
    if (norm.includes('minimal') || norm.includes('simple')) {
      if (isShapeLayer(l)) { l.shadow=undefined; l.stroke=undefined; }
...
export function generateLayoutVariants(artboards: Artboard[], layers: Layer[]): LayoutVariant[] {
  ...
  const centered = makePositions((l,i) => {
    ...
    return { x:(ab.width-l.width)/2, y:(ab.height-totalH)/2+i*(l.height+20) };
  });
  ...
  const grid = makePositions((l,i) => {
    const cols=Math.ceil(Math.sqrt(all.length)), cellW=ab.width/cols, cellH=ab.height/Math.ceil(all.length/cols);
    return { x:(i%cols)*cellW+(cellW-l.width)/2, y:Math.floor(i/cols)*cellH+(cellH-l.height)/2 };
  });
  return [
    { name:'Centered & Balanced', description:'Symmetrical centered layout', layers:centered },
    { name:'Asymmetric & Dynamic', description:'Off-center with varied angles', layers:asymmetric },
    { name:'Grid-Based & Structured', description:'Organized grid placement', layers:grid },
  ];
}
...
export function optimizeForPlatform(artboards: Artboard[], targetPlatform: Platform): Artboard[] {
  ...
  const srcW = artboards[0]?.width||800, srcH = artboards[0]?.height||600;
  const scale = Math.min(target.width,target.height)/Math.min(srcW,srcH);
  return artboards.map(ab => ({
    ...ab, width:target.width, height:target.height,
    layers: ab.layers.map(layer => {
      const l = {...layer, width:layer.width*scale, height:layer.height*scale,
        x:(layer.x/ab.width)*target.width, y:(layer.y/ab.height)*target.height};
      if (isTextLayer(l)) {
        l.fontSize *= scale;
        if (targetPlatform==='YouTube Thumbnail') { l.fontSize=Math.max(l.fontSize,24); l.fontWeight='700'; }
