...
import { log, maskEmail } from '../utils/log';
...
export class AuthService {
  ...
  async signIn(email: string, password: string): Promise<AuthResult> {
    try {
      ...
      if (useQABypass) {
        log.info('[AuthService] Using QA bypass for development', { email: maskEmail(email) });
        const mockUser: User = {
          id: 'qa-user-id',
          email: email,
          name: email.split('@')[0],
      ...
      log.info('[AuthService] Attempting Supabase sign in', { email: maskEmail(email) });
      ...
      logSecurityEvent('LOGIN_ATTEMPT', user.id, { email: maskEmail(email), success: true });
  ...
  async updateProfile(updates: Partial<Profile>): Promise<{ error: string | null }> {
    try {
      ...
      // S-3: JWT should be rotated server-side after privilege-escalating profile
      // updates (e.g., plan change to 'pro'). Client-side call:
      // await supabase.auth.getSession(); // forces token refresh

      // S-11: Verify the update actually persisted with correct user_id ownership
      const { data: verify, error: verifyErr } = await supabase
        ...
        .single();
      if (verifyErr || !verify) {
        return { error: 'Profile update verification failed' };
      }
