...
export const GlyphPalette = ({ fontFamily, onSelect, onClose }: GlyphPaletteProps) => {
    ...
    const loadFontGlyphs = async () => {
        setIsLoading(true);
        try {
            // 1. Fetch CSS from Google Fonts to find the font URL
            ...
            if (urlMatch && urlMatch[1]) {
                ...
                // 3. Load with OpenType.js
                // Note: We might run into CORS issues with direct fetch if not proxied or allowed. 
                // Google Fonts usually allows CORS.
                ...
                try {
                    ...
                    // Optimization: Limit to reasonable range or lazy load
                    // First 2000 glyphs usually cover Latin + Extended
        ...
        <div className="absolute top-full right-0 mt-3 w-64 bg-[#1e1e1e] rounded-xl shadow-2xl border border-white/10 p-3 z-50 animate-fadeIn backdrop-blur-xl flex flex-col max-h-80">
            <div className="flex justify-between items-center mb-3 border-b border-white/10 pb-2">
                ...
                <div className="flex bg-black/20 rounded-lg p-0.5">
                    <button
                        onClick={() => { setActiveTab('common'); setGlyphs(COMMON_GLYPHS); }}
