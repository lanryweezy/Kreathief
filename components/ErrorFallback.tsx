  ...
  if (variant === 'widget') {
    return (
      <div className="flex flex-col items-center justify-center p-6 bg-red-500/10 border border-red-500/20 rounded-lg h-full min-h-[200px]">
        ...
        <h3 className="text-white font-medium text-sm mb-1" title={error.message || 'Component Error'}>
          {getErrorDetails(error).message}
        </h3>
        <p className="text-red-300/70 text-xs text-center mb-3 max-w-[200px]">
  ...
  return (
    <div className="fixed inset-0 bg-[#121212] flex flex-col items-center justify-center p-4 z-overlay">
      <div className="max-w-md w-full text-center">
        ...
        <h1 className="text-2xl font-bold text-white mb-2" title={error.message || 'Error'}>
          {getErrorDetails(error).message}
        </h1>
