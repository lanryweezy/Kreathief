...
export const CropOverlay: React.FC<CropOverlayProps> = ({ zoom, canvasSize }) => {
    const {
        croppingLayerId,
        layers,
        cropArea,
    ...
    // Scale: how many canvas pixels per natural pixel
    // We use the current layer bounds and its current crop to determine this
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        // dx/dy are in canvas units, we need them in natural pixel units
        ...
        setCropArea(newArea);
    }, [isResizing, startPos, startArea, zoom, canvasScale, naturalWidth, naturalHeight, setCropArea]);
    ...
    // Position of the "full image" relative to the canvas origin
    const fullImageStyle: React.CSSProperties = {
        position: 'absolute',
        // The current layer is at (layer.x, layer.y), which corresponds to (layer.crop.x, layer.crop.y) in natural pixels
    ...
    const cropBoxStyle: React.CSSProperties = {
        position: 'absolute',
        // The crop box is relative to the full image
        ...
        <>
            ...
            <div style={{
                ...
                <div style={cropBoxStyle}>
                    {/* Handles */}
                    <div onMouseDown={(e) => handleMouseDown(e, 'nw')} className="absolute -top-1.5 -left-1.5 w-3.5 h-3.5 bg-white border-2 border-[#7d2ae8] rounded-full cursor-nw-resize z-50 shadow-md"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 'ne')} className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 bg-white border-2 border-[#7d2ae8] rounded-full cursor-ne-resize z-50 shadow-md"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 'sw')} className="absolute -bottom-1.5 -left-1.5 w-3.5 h-3.5 bg-white border-2 border-[#7d2ae8] rounded-full cursor-sw-resize z-50 shadow-md"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 'se')} className="absolute -bottom-1.5 -right-1.5 w-3.5 h-3.5 bg-white border-2 border-[#7d2ae8] rounded-full cursor-se-resize z-50 shadow-md"></div>

                    <div onMouseDown={(e) => handleMouseDown(e, 'n')} className="absolute -top-1 left-1/2 -translate-x-1/2 w-8 h-1.5 bg-white border border-[#7d2ae8] rounded-full cursor-ns-resize z-40 shadow-sm"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 's')} className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-8 h-1.5 bg-white border border-[#7d2ae8] rounded-full cursor-ns-resize z-40 shadow-sm"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 'w')} className="absolute top-1/2 -translate-y-1/2 -left-1 w-1.5 h-8 bg-white border border-[#7d2ae8] rounded-full cursor-ew-resize z-40 shadow-sm"></div>
                    <div onMouseDown={(e) => handleMouseDown(e, 'e')} className="absolute top-1/2 -translate-y-1/2 -right-1 w-1.5 h-8 bg-white border border-[#7d2ae8] rounded-full cursor-ew-resize z-40 shadow-sm"></div>
