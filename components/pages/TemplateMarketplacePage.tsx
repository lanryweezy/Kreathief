    ...
    <div className="flex flex-col bg-surface-dark-0 min-h-[600px] text-white p-6 md:p-10 border border-white/5 rounded-3xl relative overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center gap-2 bg-white/5 p-1 rounded-xl border border-white/5">
          {CATS.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${category === c ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
              {c}
            </button>
          ))}
      ...
      <div className="flex items-center gap-2 mb-6 relative z-10">
        ...
        {[
          ...
          <button
            key={o.id}
            onClick={() => setSort(o.id as SortOpt)}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${sort === o.id ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
          >
            {o.l}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 flex-1 relative z-10">
        {templates.map((t) => (
          <div
            ...
            className="group bg-surface-dark-1 border border-white/5 hover:border-white/20 rounded-2xl overflow-hidden cursor-pointer flex flex-col transition-all duration-300 shadow-xl"
          >
            <div className="aspect-[4/3] bg-surface-dark-0 flex items-center justify-center relative overflow-hidden border-b border-white/5">
              {t.thumbnailUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-surface-dark-1 to-surface-dark-2">
                  <Icons.Templates className="w-12 h-12 text-gray-700" />
                </div>
              )}
              <div className="absolute top-2 left-2 flex gap-1">
                {t.tags.slice(0, 2).map((tag) => (
                  <span
                    key={tag}
                    className="text-[9px] font-bold uppercase bg-black/60 backdrop-blur-md px-2 py-0.5 rounded text-white border border-white/10"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <div className="p-4 flex-1 flex flex-col justify-between">
              ...
              <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5">
                <div className="flex gap-3 text-[10px] font-bold text-gray-400">
                  <span
                    onClick={(e) => like(e, t.id)}
                    className="flex items-center gap-1 cursor-pointer hover:text-red-400 transition-colors"
                  >
                    <Icons.Heart className="w-3 h-3" /> {t.likes}
                  </span>
                  <span className="flex items-center gap-1">
                    <Icons.History className="w-3 h-3" /> {t.downloads}
                  </span>
                </div>
                <span className="text-[9px] font-bold uppercase bg-white/5 border border-white/10 text-brand-400 px-2 py-0.5 rounded-full">
                  {t.category}
                </span>
              </div>
            </div>
      ...
      {!loading && !templates.length && (
        <div className="flex flex-col items-center justify-center py-16 text-gray-500">
          <Icons.Templates className="w-16 h-16 mb-4 opacity-30" />
          <p className="text-sm font-medium">No templates found</p>
          <p className="text-xs mt-1">Try adjusting filters or submit a new template</p>
        </div>
      )}
      <TemplateSubmitModal
        isOpen={showSubmit}
