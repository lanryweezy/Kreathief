 ...
 */
export const SaveIndicator: React.FC<SaveIndicatorProps> = ({ status, lastSaved, onManualSave }) => {
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      <Icon className={`w-4 h-4 ${status === 'saving' ? 'animate-spin' : ''}`} aria-hidden="true" />
      <span className="text-xs font-medium">{config.text}</span>

      {status === 'saved' && timeAgo && <span className="text-xs text-gray-500">• {timeAgo}</span>}

      {status === 'unsaved' && onManualSave && (
        <button onClick={onManualSave} className="ml-1 text-xs underline hover:no-underline" aria-label="Save now">
          Save
        </button>
      )}
    </div>
