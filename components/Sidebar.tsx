...
export const Sidebar: React.FC<SidebarProps> = React.memo(({ isCollapsed, isAutoCollapsed, onToggleCollapse, onExpand }) => {
  ...
  return (
    <div id="sidebar" className="w-[72px] bg-[#0e1318] flex flex-col items-center py-4 z-30 shrink-0 border-r border-[#1f1f1f] h-full overflow-y-auto custom-scrollbar scrollbar-hide">
      <div className="flex flex-col items-center w-full gap-4" role="tablist">
        {toolGroups.map((group, gIdx) => (
          <div key={group.name} className="w-full flex flex-col items-center gap-1">
            {gIdx > 0 && <div className="w-8 h-px bg-white/5 mb-2" />}
