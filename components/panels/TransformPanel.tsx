...
export const TransformPanel: React.FC = () => {
  ...
  const handleDistribute = useCallback(
    (type: 'horizontal' | 'vertical') => {
      ...
      const totalDist = type === 'horizontal' ? last.x - first.x : last.y - first.y;
  ...
  return (
    <div className="bg-surface-dark-3 rounded-xl border border-gray-700 p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-4">
        <div>
          <label className="text-[10px] font-black text-gray-500 block mb-1 uppercase tracking-widest">
            X Position
          </label>
          <input
            type="text"
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              const val = parseInt(e.target.value);
              if (!isNaN(val)) {
                handleBatchUpdate('cornerRadius', val);
              }
            }}
            placeholder="0"
            className="w-full bg-surface-dark-4 border border-gray-600 rounded px-2 py-1.5 text-xs text-white focus:border-brand-600 outline-none"
