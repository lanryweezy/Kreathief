...
export const UploadsPanel: React.FC<UploadsPanelProps> = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-dark-2 overflow-hidden">
      <PanelHeader title="Media Library" icon={<Icons.Uploads className="w-5 h-5" />} />
        ...
        <div className="grid grid-cols-2 gap-3 overflow-y-auto custom-scrollbar flex-1 pb-4">
          {filteredUploads && filteredUploads.length > 0 ? (
            filteredUploads.map((url, idx) => (
              ...
              >
                <img src={url} alt="Uploaded image" className="w-full h-full object-cover pointer-events-none" loading="lazy" />
