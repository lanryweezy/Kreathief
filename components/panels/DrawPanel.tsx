...
export const DrawPanel: React.FC<DrawPanelProps> = ({
  ...
  return (
    <div className="flex flex-col h-full p-4">
      ...
      <div className="mb-6">
        <label className="text-xs font-bold text-gray-400 mb-3 block">Brush Type</label>
        <div className="grid grid-cols-2 gap-2">
          {[
            { id: BrushType.BASIC, name: 'Basic', icon: Icons.Brush },
            { id: BrushType.CALLIGRAPHY, name: 'Calligraphy', icon: Icons.Edit },
            { id: BrushType.OIL, name: 'Oil Brush', icon: Icons.Droplet },
            ...
            <button
              ...
              className={`flex items-center gap-2 p-2 rounded border transition-all ${brushType === type.id ? 'bg-[#7d2ae8]/20 border-[#7d2ae8] text-white' : 'bg-[#252627] border-gray-700 text-gray-400 hover:border-gray-600'}`}
            >
              {(type.icon as any) ? <type.icon className="w-4 h-4" /> : <Icons.Brush className="w-4 h-4" />}
