...
const BRUSH_PREVIEWS: Record<string, React.ReactNode> = {
  [BrushType.BASIC]: <path d="M5 25 Q 50 5 95 25" stroke="currentColor" strokeWidth="4" fill="none" strokeLinecap="round" />,
  [BrushType.CALLIGRAPHY]: <path d="M5 25 Q 50 5 95 25" stroke="currentColor" strokeWidth="4" fill="none" transform="skewX(-20)" />,
  [BrushType.OIL]: <path d="M5 25 Q 50 5 95 25" stroke="currentColor" strokeWidth="6" strokeDasharray="1 1" fill="none" filter="url(#oilFilter)" />,
  [BrushType.CRAYON]: <path d="M5 25 Q 50 5 95 25" stroke="currentColor" strokeWidth="4" strokeDasharray="0.5 2" fill="none" />,
  ...
  return (
    <div className="flex flex-col h-full p-4 overflow-y-auto custom-scrollbar pb-10">
      ...
      <div className="mb-6 p-4 bg-[#252627] rounded-lg border border-gray-700 relative overflow-hidden">
        <div className={`absolute top-0 right-0 p-2 rounded-bl-lg text-[10px] font-bold ${isDrawing ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-400'}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            >
              ...
              <svg className="absolute top-0 right-0 w-full h-full opacity-30 pointer-events-none" viewBox="0 0 100 50" preserveAspectRatio="none">
                <defs>
                  <filter id="oilFilter"><feTurbulence type="fractalNoise" baseFrequency="0.1" numOctaves="3" result="noise" /><feDisplacementMap in="SourceGraphic" in2="noise" scale="5" /></filter>
                  <filter id="watercolorFilter"><feTurbulence type="fractalNoise" baseFrequency="0.05" numOctaves="5" /><feGaussianBlur stdDeviation="2" /></filter>
                </defs>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#1e1e1e] p-3 rounded-lg border border-gray-800 space-y-4">
          ...
          <div>
            <div className="flex justify-between mb-1.5">
              ...
              <span className="text-[10px] text-gray-500 bg-gray-800 px-1.5 rounded">{Math.round(brushOpacity * 100)}%</span>
            </div>
            <input
              type="range"
              min="0.1"
        ...
        <div className="bg-[#1e1e1e] p-3 rounded-lg border border-gray-800 space-y-4">
          <label className="text-[10px] font-bold text-gray-400 uppercase block border-b border-gray-700 pb-2">Stroke Settings</label>
          ...
          <div>
            ...
            <input
              type="range" min="0" max="100" value={smoothing} onChange={(e) => setSmoothing(parseInt(e.target.value))}
          ...
          <div>
            ...
            <input
              type="range" min="0" max="100" value={jitter} onChange={(e) => setJitter(parseInt(e.target.value))}
