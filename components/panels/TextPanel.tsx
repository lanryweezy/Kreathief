...
export const TextPanel: React.FC = () => {
  ...
  return (
    <div data-testid="text-panel" className="flex flex-col h-full bg-surface-dark-2 overflow-hidden">
      <PanelHeader title="Typography" icon={<Icons.Text className="w-5 h-5" />} />
        ...
        {selectedTextLayer && (
          <>
            <div className="mb-4">
              <button
                onClick={() => {
                  useStore.getState().syncTextStyleAcrossProject(selectedTextLayer.id);
                  useStore
                    .getState()
                    .addToast(
                      'Synced text style to all matching elements across project! (Canva cannot do this)',
                      'success'
                    );
                }}
                className="w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 text-indigo-300 transition-colors"
                title="Apply this text style to all other text layers using the same font across the project"
              >
                <Icons.Layers className="w-4 h-4" />
                <span className="text-xs font-bold">Sync Style to All Matching</span>
              </button>
            </div>
            <div className="mb-4">
