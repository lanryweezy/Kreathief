...
interface Col { id: string; name: string; created_at: string; }
interface ColItem { id: string; asset_url: string; thumbnail_url: string | null; }
const inputCls = 'flex-1 bg-surface-dark-3 border border-gray-700 rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-accent';
...
export const AssetCollections: React.FC = () => {
  ...
  const loadCols = useCallback(async () => {
    ...
    if (user) {
      const { data } = await supabase.from('user_collections').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).catch(() => ({ data: null }));
      setCols((data as Col[]) || []);
    }
    setLoading(false);
  }, []);
  ...
  const loadItems = useCallback(async (cid: string) => {
    const { data } = await supabase.from('collection_items').select('*').eq('collection_id', cid).order('position').catch(() => ({ data: null }));
    setItems((data as ColItem[]) || []);
  }, []);
  useEffect(() => { loadCols(); }, [loadCols]);
  useEffect(() => { selId ? loadItems(selId) : setItems([]); }, [selId, loadItems]);
  const create = async () => {
    if (!newName.trim()) return;
    const { data: { user } } = await supabase.auth.getUser().catch(() => ({ data: { user: null } }));
    if (!user) return;
    const { data } = await supabase.from('user_collections').insert({ user_id: user.id, name: newName.trim() }).select().single().catch(() => ({ data: null }));
    if (data) { setCols((p) => [data as Col, ...p]); setNewName(''); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#13161a] p-4 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {loading ? (
          <div className="flex items-center justify-center h-40"><div className="animate-spin w-6 h-6 border-3 border-accent border-t-transparent rounded-full" /></div>
        ) : cols.length === 0 ? (
          <div className="text-center text-gray-500 mt-10"><Icons.Folder className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="text-sm">No collections yet</p></div>
        ) : (
          <div className="space-y-2">
            {cols.map((col) => (
              <div key={col.id} className={`rounded-lg border cursor-pointer transition-all ${selId === col.id ? 'bg-accent/10 border-accent' : 'bg-surface-dark-3 border-gray-700 hover:border-gray-600'}`} onClick={() => setSelId(selId === col.id ? null : col.id)}>
                ...
                {selId === col.id && items.length > 0 && (
                  <div className="grid grid-cols-3 gap-1 px-3 pb-3">
                    {items.map((it) => (
                      <div key={it.id} className="aspect-square rounded bg-surface-dark-4 overflow-hidden" draggable onDragStart={(e) => e.dataTransfer.setData('application/json', JSON.stringify(it))}>
                        <img src={it.thumbnail_url || it.asset_url} className="w-full h-full object-cover" alt="" loading="lazy" />
                      </div>
                    ))}
                  </div>
                )}
      ...
      <ConfirmModal isOpen={!!del} onClose={() => setDel(null)} onConfirm={deleteCol} title="Delete Collection" message={`Delete "${del?.name}"? This cannot be undone.`} confirmLabel="Delete" variant="danger" />
    </div>
  );
};
