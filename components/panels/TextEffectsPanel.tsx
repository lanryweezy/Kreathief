...
import { TextLayer, AdvancedShadow } from '../../types';
...
const TEXTURE_PRESETS = [
    { name: "Noise", url: "data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.4'/%3E%3C/svg%3E" },
    { name: "Paper", url: "data:image/svg+xml,%3Csvg width='200' height='200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='paper'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.04' numOctaves='5' stitchTiles='stitch'/%3E%3CfeDiffuseLighting lighting-color='%23f2ebd4' surfaceScale='2'%3E%3CfeDistantLight azimuth='45' elevation='60'/%3E%3C/feDiffuseLighting%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23paper)' opacity='0.3'/%3E%3C/svg%3E" },
    { name: "Grunge", url: "data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='grunge'%3E%3CfeTurbulence type='turbulence' baseFrequency='0.05' numOctaves='2' result='turbulence'/%3E%3CfeDisplacementMap in2='turbulence' in='SourceGraphic' scale='50' xChannelSelector='R' yChannelSelector='G'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' fill='%23000' opacity='0.3' filter='url(%23grunge)'/%3E%3C/svg%3E" },
    { name: "Halftone", url: "data:image/svg+xml,%3Csvg width='10' height='10' xmlns='http://www.w3.org/2000/svg'%3E%3Cpattern id='halftone' width='10' height='10' patternUnits='userSpaceOnUse'%3E%3Ccircle cx='5' cy='5' r='2' fill='%23000' opacity='0.4'/%3E%3C/pattern%3E%3Crect width='100%25' height='100%25' fill='url(%23halftone)'/%3E%3C/svg%3E" },
];
...
export const TextEffectsPanel: React.FC<TextEffectsPanelProps> = ({
    ...
    const toggleCut = () => {
        const newCuts = decorations.cuts && decorations.cuts.length > 0 ? [] : [{ position: 40, width: 2, angle: 45 }];
    ...
    const toggleLine = (type: 'top' | 'middle' | 'bottom') => {
        ...
        const newLines = exists
            ...
            : [...lines, { type, color: selectedLayer.color, width: 2 }];
    ...
    return (
        <div className="flex flex-col h-full bg-[#13161a] p-4 overflow-y-auto custom-scrollbar">
            ...
            <section className="mb-8">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4 block">Transformations</label>
                    ...
                    <div className="space-y-4 bg-[#1e1e1e] p-3 rounded-xl border border-gray-800">
                        <div>
                            <div className="flex justify-between mb-1.5">
                                ...
                                <span className="text-[10px] text-purple-400 font-mono">{transformIntensity}%</span>
                            </div>
                            <input
                                type="range"
                                min="0"
                                ...
                                onChange={(e) => handleUpdateTransform({ transformIntensity: parseInt(e.target.value) })}
                        ...
                        {transformType === 'circle' && (
                            <div>
                                <div className="flex justify-between mb-1.5">
                                    ...
                                    <span className="text-[10px] text-purple-400 font-mono">{transformDirection}°</span>
                                </div>
                                <input
                                    type="range"
                                    min="0"
                                    ...
                                    onChange={(e) => handleUpdateTransform({ transformDirection: parseInt(e.target.value) })}
                                    className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500"
                                />
                            </div>
                        )}
            ...
            <section className="mb-8">
                <div className="flex justify-between items-center mb-4">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Shadow Effects</label>
                    ...
                    {advancedShadows.map((s, idx) => (
                        <div key={idx} className="bg-[#1e1e1e] p-3 rounded-xl border border-gray-800">
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-[10px] font-bold text-purple-400 uppercase">{s.type} Shadow</span>
                                <button onClick={() => handleRemoveShadow(idx)} className="text-gray-600 hover:text-red-400">
                                ...
                                {(s.type === 'block' || s.type === '3d') ? (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            type="number"
                                            value={s.distance}
                                            onChange={(e) => handleUpdateShadow(idx, { distance: parseInt(e.target.value) })}
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            type="number"
                                            value={s.offsetX}
                                            onChange={(e) => handleUpdateShadow(idx, { offsetX: parseInt(e.target.value), offsetY: parseInt(e.target.value) })}
                                            className="w-full bg-gray-900 border border-gray-800 rounded px-1 py-0.5 text-[10px] text-white"
                                        />
                                    </div>
                                )}
            ...
            {/* Decorations */}
            <section>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4 block">Decorations</label>
                ...
                <div className="mt-6">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 block">Textures</label>
                    <div className="grid grid-cols-4 gap-2">
                        {TEXTURE_PRESETS.map((tex, i) => (
                            <button
                                key={i}
                                onClick={() => handleApplyTexture(tex.url)}
                                className={`aspect-square rounded-lg border-2 transition-all flex items-center justify-center overflow-hidden ${decorations.textures?.includes(tex.url) ? 'border-purple-500 ring-2 ring-purple-500/20' : 'border-gray-800 hover:border-gray-700'}`}
                                title={tex.name}
                            >
                ...
                <div className="mt-6">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 block">Dynamic Lines</label>
