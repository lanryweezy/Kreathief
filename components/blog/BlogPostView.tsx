            ...
            <div className="max-w-[1200px] mx-auto px-6 relative z-10 flex flex-col md:flex-row gap-12">
                ...
                <aside className="w-full md:w-64 flex-shrink-0">
                    <div className="sticky top-32">
                        <Link to="/blog" className="inline-flex items-center gap-2 text-gray-500 hover:text-white transition-colors mb-12 group text-xs uppercase tracking-widest font-bold">
                ...
                <main className="flex-1 max-w-3xl">
                    ...
                    <article className="prose prose-invert prose-purple max-w-none prose-lg">
                        <div className="text-gray-400 leading-[1.8] text-xl font-medium space-y-8">
                            {post.content.split('\n').map((line, i) => {
                                ...
                                if (trimmed.startsWith('## ')) {
                                    return <h2 key={i} className="text-3xl font-bold text-white mt-14 mb-6">{formatText(trimmed.replace('## ', ''))}</h2>;
                                }
                                if (trimmed.startsWith('### ')) {
                                    return <h3 key={i} className="text-2xl font-bold text-purple-400 mt-10 mb-4">{formatText(trimmed.replace('### ', ''))}</h3>;
                                }
                                if (trimmed.startsWith('- ')) {
                                    return <li key={i} className="ml-8 mb-3 list-disc marker:text-purple-500 pl-2">{formatText(trimmed.replace('- ', ''))}</li>;
                                }
                                if (trimmed.startsWith('![') && trimmed.includes('](')) {
                                    ...
                                    return (
                                        <figure key={i} className="my-16 relative group">
                                            ...
                                            <img src={src} alt={alt} className="w-full rounded-2xl relative z-10 border border-white/10 shadow-2xl" />
                                            <figcaption className="text-center text-sm text-gray-500 mt-4 font-medium italic">{alt}</figcaption>
                                        </figure>
                                    );
                                }
                                return <p key={i} className="mb-6">{formatText(trimmed)}</p>;
                            })}
                        </div>
                    </article>
                    ...
                    <div className="mt-32 p-12 rounded-[40px] border border-white/10 bg-gradient-to-br from-[#111] to-[#050505] text-center relative overflow-hidden">
                        ...
                        <p className="text-xl text-gray-400 mb-10 relative z-10">Join thousands of creative professionals building without limits.</p>
                        <Link
                            to="/"
