...
const CATEGORIES = [
  ...
  { id: 'kreathief', label: 'KreaThief', icon: 'Star' },
  ...
  { id: 'ai-art', label: 'AI Art', icon: 'Brush' },
...
export function WorkflowPresets({ onSelect }: WorkflowPresetsProps) {
  ...
  const filteredPresets = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      presets = presets.filter((p) => fuzzyMatch(searchQuery, p.name) || fuzzyMatch(searchQuery, p.description));
    }
  ...
  return (
    <div className="flex flex-col h-full bg-surface-dark-1">
      ...
      <div className="flex gap-1 p-2 border-b border-white/10 overflow-x-auto no-scrollbar">
        {CATEGORIES.map((cat) => {
          const Icon = (Icons as any)[cat.icon] || Icons.Zap;
          return (
            <button
              key={cat.id}
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
        {filteredPresets.map((preset) => {
          const PresetIcon = (Icons as any)[preset.icon] || Icons.Box;
          return (
            <button
              key={preset.id}
            ...
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-surface-dark-3 border border-white/5 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                  <PresetIcon className="w-5 h-5 text-white/80 group-hover:text-white" />
                </div>
