...
export const TransformTools = React.memo(({ selectedLayer }: TransformToolsProps) => {
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <IconButton onClick={() => handleUpdateLayer({ rotation: (selectedLayer.rotation + 90) % 360 })} title="Rotate 90°">
                <Icons.RotateCw className="w-4 h-4" />
            </IconButton>

            {showAdvanced && (
                <div className="flex items-center gap-3 animate-fadeIn border-l border-white/10 ml-2 pl-3">
                    <CompactInput label="R" value={selectedLayer.rotation} onChange={(e: any) => handleUpdateLayer({ rotation: parseInt(e.target.value) })} min={0} max={360} width="w-9" />
                    <CompactInput label="RX" value={(selectedLayer as any).rotateX || 0} onChange={(e: any) => handleUpdateLayer({ rotateX: parseInt(e.target.value) })} min={-180} max={180} width="w-8" />
                    <CompactInput label="RY" value={(selectedLayer as any).rotateY || 0} onChange={(e: any) => handleUpdateLayer({ rotateY: parseInt(e.target.value) })} min={-180} max={180} width="w-8" />
                    <CompactInput label="P" value={(selectedLayer as any).perspective || 0} onChange={(e: any) => handleUpdateLayer({ perspective: parseInt(e.target.value) })} min={0} max={2000} width="w-10" />
                </div>
            )}
