...
export const MaskTools = React.memo(
  ({ layer, onUpdateLayer, documentColors, isPro: _isPro, onOpenPricing: _onOpenPricing }: MaskToolsProps) => {
    ...
    const updateGradient = (changes: any) => {
      const currentGradient = (layer as any).imageFill?.gradientOverlay ||
        (layer as any).backgroundGradient || {
          type: 'linear',
          angle: 0,
          colors: [
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-2">
          <label className="cursor-pointer" title="Upload image mask">
            ...
            <div
              className={`p-1.5 rounded-lg border border-dashed transition-all ${(layer as any).imageFill ? 'bg-[#7d2ae8] border-[#7d2ae8] text-white shadow-lg shadow-[#7d2ae8]/30' : 'bg-black/20 border-white/20 text-gray-400 hover:border-[#7d2ae8]/50 hover:text-white'}`}
            >
          ...
          {(layer as any).imageFill && (
            <div className="relative" ref={settingsRef}>
              <IconButton onClick={() => setShowSettings(!showSettings)} active={showSettings} title="Mask Settings">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-[9px] text-white font-mono">
                          {Math.round((layer as any).imageFill.scale * 100)}%
                        </span>
                      </div>
                      <input
                        type="range"
                        ...
                        max="3"
                        step="0.1"
                        value={(layer as any).imageFill.scale}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          type="number"
                          value={(layer as any).imageFill.offsetX}
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="number"
                          value={(layer as any).imageFill.offsetY}
                    ...
                    {(layer as any).imageFill.gradientOverlay && (
                      <div className="space-y-3 pl-1 border-l-2 border-[#7d2ae8]/20">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-white font-mono">
                            {(layer as any).imageFill.gradientOverlay.angle}°
                          </span>
                        </div>
                        <input
                          type="range"
                          min="0"
                          max="360"
                          value={(layer as any).imageFill.gradientOverlay.angle || 0}
