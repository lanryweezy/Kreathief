...
const TemplateSubmitModal: React.FC<TemplateSubmitModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !file.name.endsWith('.json')) { setError('Upload a valid JSON file'); return; }
    const reader = new FileReader();
    reader.onload = (ev) => {
      try { setTemplateData(JSON.parse(ev.target?.result as string)); setError(''); }
  ...
  const handleSubmit = async () => {
    if (!title.trim() || !category || !templateData) { setError('Title, category, and template file required'); return; }
    setSubmitting(true);
    const data: SubmitTemplateData = {
      title: title.trim(), description: description.trim(), category,
      tags: tagsInput.split(',').map((t) => t.trim()).filter(Boolean), templateData,
    };
    const result = await templateMarketplace.submitTemplate(data);
    setSubmitting(false);
    if (result) { resetForm(); onClose(); onSuccess?.(); }
    else setError('Failed to submit template');
  };

  const resetForm = () => { setTitle(''); setDescription(''); setCategory(''); setTagsInput(''); setTemplateData(null); setError(''); };
    ...
    <ModalWrapper isOpen={isOpen} onClose={onClose} maxWidth="max-w-xl">
      <div className="p-6 md:p-8">
        ...
        <div className="space-y-4">
          <Input label="Template Title" placeholder="My Awesome Template" value={title} onChange={(e) => setTitle(e.target.value)} />
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-gray-400">Description</label>
            <textarea placeholder="Describe your template..." value={description} onChange={(e) => setDescription(e.target.value)}
              rows={3} className="w-full px-3 py-2 text-sm rounded-xl bg-surface-dark-3 text-white border border-white/10 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-400/50 resize-none" />
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <Input label="Tags (comma-separated)" placeholder="poster, neon, minimal" value={tagsInput} onChange={(e) => setTagsInput(e.target.value)} />
          <div className="flex flex-col gap-1.5">
            ...
            <button onClick={() => fileInputRef.current?.click()}
              ...
              {templateData ? (
                <div className="flex items-center justify-center gap-2 text-brand-400">
                  <Icons.Check className="w-5 h-5" /><span className="text-sm font-medium">Template loaded</span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2 text-gray-500 group-hover:text-gray-400">
                  <Icons.Upload className="w-8 h-8" /><span className="text-sm">Click to upload template JSON</span>
                </div>
              )}
            </button>
          </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <Button onClick={handleSubmit} loading={submitting} disabled={!title.trim() || !category || !templateData} className="flex-1">
            Submit for Review
          </Button>
        </div>
      </div>
