  ...
  return (
    <div className="fixed inset-0 z-[2100] bg-black/90 text-white" onClick={() => setShow(false)}>
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
        ...
        <button className="text-gray-400 hover:text-white" aria-label="Close" onClick={() => setShow(false)}>×</button>
      </div>
      <div className="flex gap-6 p-6" onClick={(e)=>e.stopPropagation()}>
        <div className="flex-1 bg-[#0f1216] rounded-xl p-4 border border-white/10">
          <div className="flex items-center justify-between mb-2"><div className="text-sm opacity-70">Left</div><select className="bg-[#14181d] border border-white/10 rounded px-2 py-1 text-sm" value={leftId} onChange={(e)=> setLeftId(e.target.value as any)}><option value="live">Current (Live)</option> </select></div>
          {leftAb ? (
            <div style={{ width: leftAb.width, height: leftAb.height }} className="overflow-hidden">
              <StaticLayerRenderer layers={leftAb.layers} scale={Math.min(1, (Math.min(window.innerWidth/2-96, 640))/Math.max(1,leftAb.width))} />
            </div>
        ...
        <div className="flex-1 bg-[#0f1216] rounded-xl p-4 border border-white/10">
          <div className="flex items-center justify-between mb-2">
            ...
            <select className="bg-[#14181d] border border-white/10 rounded px-2 py-1 text-sm" value={rightId||rightSnapshot?.id||''} onChange={(e)=>setRightId(e.target.value)}>
              {snapshots?.map((s)=> (
                <option key={s.id} value={s.id}>{s.name || new Date(s.timestamp).toLocaleString()}</option>
              ))}
            </select>
          </div>
          {rightAb ? (
            <div style={{ width: rightAb.width, height: rightAb.height }} className="overflow-hidden">
              <StaticLayerRenderer layers={rightAb.layers} scale={Math.min(1, (Math.min(window.innerWidth/2-96, 640))/Math.max(1,rightAb.width))} />
            </div>
