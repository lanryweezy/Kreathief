  ...
  return (
    <div className="h-full flex flex-col bg-slate-900 text-white">
      <div className="p-4 border-b border-slate-700">
        ...
        <p className="text-xs text-slate-400 mt-1">Discuss this design with your team.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div
              key={comment.id}
              className={`flex gap-3 ${comment.userId === currentUser.id ? 'flex-row-reverse' : ''}`}
            >
              ...
              <div
                className={`flex flex-col max-w-[80%] ${comment.userId === currentUser.id ? 'items-end' : 'items-start'}`}
              >
