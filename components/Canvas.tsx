...
import { GeneratedImage, TextLayer, ShapeLayer, ImageLayer, Layer, CanvasFilters, CanvasSize, User } from '../types';
...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  // Interaction State
  const [dragState, setDragState] = useState<{ isDragging: boolean, startX: number, startY: number, layerInitialX: number, layerInitialY: number } | null>(null);
  ...
  const handleMouseDownLayer = (e: React.MouseEvent, layer: Layer) => {
    ...
    setDragState({
        ...
        startX: e.clientX,
        startY: e.clientY,
        layerInitialX: layer.x,
        layerInitialY: layer.y
    });
  };
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (dragState?.isDragging && selectedLayerId) {
        ...
        let newX = dragState.layerInitialX + dx;
        let newY = dragState.layerInitialY + dy;

        // Snapping Logic
        const layer = layers.find(l => l.id === selectedLayerId);
        if (layer) {
            const { snapX, snapY, newX: snappedX, newY: snappedY } = getSnapLines({ ...layer, x: newX, y: newY }, newX, newY);
            setSnapLines({ vertical: snapX, horizontal: snapY });
            if (snapX !== undefined) newX = snappedX;
            if (snapY !== undefined) newY = snappedY;
        }
  ...
  const handleDrawingMouseMove = (e: React.MouseEvent) => {
     ...
     const y = (e.clientY - rect.top) / zoom;
     const ctx = drawingCanvasRef.current.getContext('2d');
     if (ctx) { ctx.lineTo(x, y); ctx.stroke(); setDrawingState({ ...drawingState, lastX: x, lastY: y }); }
  };
      ...
      >
         ...
         >
             ...
             >
                ...
                {/* Layers */}
                {shapeLayers.map(l => (
                    <ShapeLayerItem key={l.id} layer={l} isSelected={selectedLayerId === l.id} isHovered={hoveredLayerId === l.id} onMouseDown={handleMouseDownLayer} onMouseEnter={setHoveredLayerId} onMouseLeave={() => setHoveredLayerId(null)} onResize={handleResizeStart} onRotate={handleRotateStart} onContextMenu={handleContextMenu} onDrop={handleDropShape} />
                ))}
                {imageLayers.map(l => (
                    <ImageLayerItem key={l.id} layer={l} isSelected={selectedLayerId === l.id} isHovered={hoveredLayerId === l.id} onMouseDown={handleMouseDownLayer} onMouseEnter={setHoveredLayerId} onMouseLeave={() => setHoveredLayerId(null)} onResize={handleResizeStart} onRotate={handleRotateStart} onContextMenu={handleContextMenu} />
                ))}
                {textLayers.map(l => (
                ...
                {isDrawing && <canvas ref={drawingCanvasRef} width={canvasSize.width} height={canvasSize.height} className="absolute inset-0 z-[100] cursor-crosshair" onMouseDown={handleDrawingMouseDown} onMouseMove={handleDrawingMouseMove} onMouseUp={handleDrawingMouseUp} onMouseLeave={handleDrawingMouseUp} />}
                {isProcessing && (
