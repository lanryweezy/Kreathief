...
export const ShortcutOverlay: React.FC<ShortcutOverlayProps> = ({ isOpen, onClose }) => {
    ...
    const shortcuts = [
        ...
        { key: 'Ctrl + C', desc: 'Copy layer' },
        { key: 'Ctrl + V', desc: 'Paste layer' },
        { key: 'Ctrl + D', desc: 'Duplicate layer' },
        ...
        { key: 'Ctrl + Click', desc: 'Multi-select layers' },
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-[#1e1e1e] border border-gray-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-1 gap-2">
                        {shortcuts.map((shortcut, index) => (
                            <div key={index} className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0 hover:bg-white/5 px-2 rounded transition-colors group">
                                <span className="text-gray-300 text-sm font-medium">{shortcut.desc}</span>
                                <div className="flex items-center gap-1">
                                    {shortcut.key.split(' + ').map((k, i) => (
                                        <React.Fragment key={i}>
                                            {i > 0 && <span className="text-gray-600 text-xs">+</span>}
                                            <kbd className="px-2 py-1 bg-[#252627] border border-gray-700 rounded text-xs text-gray-200 font-mono shadow-sm group-hover:border-gray-500 transition-colors">
                                                {k}
                                            </kbd>
                                        </React.Fragment>
                                    ))}
                ...
                <div className="p-4 bg-[#13161a] border-t border-gray-700 text-center">
                    <p className="text-xs text-gray-500">
                        Hold <kbd className="px-1.5 py-0.5 bg-[#252627] border border-gray-700 rounded text-[10px] text-gray-400">Space</kbd> to pan the canvas at any time.
                    </p>
                </div>
            </div>
        </div>
