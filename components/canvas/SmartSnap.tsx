...
export const SmartSnap: React.FC<SmartSnapProps> = ({ layers, selectedIds, zoom }) => {
    ...
    otherLayers.forEach((layer) => {
      ...
      if (Math.abs(bounds.top - lb.top) < threshold)
        indicators.push({
          ...
          y1: lb.top,
          x2: Math.max(bounds.right, lb.right),
          y2: lb.top,
          label: 'Align top',
        });
  ...
  if (selectedLayers.length >= 3 && otherLayers.length === 0) {
    ...
    const isEqualSpacing = sorted.every(
      (l, i) => i === 0 || Math.abs(l.x - (sorted[i - 1].x + sorted[i - 1].width) - spacing) < 2
    );
    if (isEqualSpacing) {
      ...
      indicators.push({
        type: 'horizontal',
        x1: first.x,
        y1: first.y + first.height + 10,
        x2: last.x + last.width,
        y2: last.y + last.height + 10,
        label: 'Equal spacing',
      });
    }
  }
