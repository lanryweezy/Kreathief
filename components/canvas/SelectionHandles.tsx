...
const HS = 'absolute w-2 h-2 bg-white border-[1.5px] border-brand-600 rounded-sm pointer-events-auto transition-transform duration-150 hover:scale-125';
...
export const SelectionHandles = React.memo(({ layer, multi, onResize, onRotate, onKeyboardResize }: Props) => {
  ...
  const key = (e: React.KeyboardEvent, h: Handle) => {
    ...
    const v = d[e.key]; if (!v) return; e.preventDefault();
    onKeyboardResize(layer, h, v[0], v[1], e.shiftKey);
  };
  const h = (handle: Handle, cls: string) => (
    <div key={handle} tabIndex={0} onKeyDown={(e) => key(e, handle)}
      onPointerDown={(e) => onResize(e, layer, handle)} style={r} className={`${HS} ${cls}`} />
  );

  return (
    <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 50 }}>
      <div className={`absolute -inset-[2px] ${multi ? 'border-[1px] border-dashed border-brand-600' : 'border-[1px] border-brand-600 shadow-[0_0_0_2px_rgba(255,255,255,0.8)]'} transition-all`}
      ...
      {!layer.locked && <>
        ...
        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-auto z-50" style={r}>
          <div className="w-px h-4 bg-brand-600" />
          <div onPointerDown={(e) => onRotate(e, layer)}
            onDoubleClick={(e) => { e.stopPropagation(); updateLayer(layer.id, { rotation: 0 }); }}
