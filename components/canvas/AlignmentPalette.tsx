...
export const AlignmentPalette: React.FC<AlignmentPaletteProps> = React.memo(({ selectedLayerIds, layers, zoom }) => {
  ...
  return (
    ...
    >
      <Btn
        icon={<Icons.AlignLeft className="w-3.5 h-3.5" />}
        tip="Align Left"
        onClick={act(() => alignLayers('left'))}
      />
      <Btn
        icon={<Icons.AlignCenter className="w-3.5 h-3.5" />}
        tip="Align Center"
        onClick={act(() => alignLayers('center'))}
      />
      <Btn
        icon={<Icons.AlignRight className="w-3.5 h-3.5" />}
        tip="Align Right"
        onClick={act(() => alignLayers('right'))}
      />
      <div className="w-px h-4 bg-white/10 mx-0.5" />
      <Btn icon={<Icons.AlignTop className="w-3.5 h-3.5" />} tip="Align Top" onClick={act(() => alignLayers('top'))} />
      <Btn
        icon={<Icons.AlignMiddle className="w-3.5 h-3.5" />}
        tip="Align Middle"
        onClick={act(() => alignLayers('middle'))}
      />
      <Btn
        icon={<Icons.AlignBottom className="w-3.5 h-3.5" />}
        tip="Align Bottom"
        onClick={act(() => alignLayers('bottom'))}
      />
      <div className="w-px h-4 bg-white/10 mx-0.5" />
      <Btn
        icon={<DistributeH className="w-3.5 h-3.5" />}
        tip="Distribute H"
        onClick={act(() => distributeLayers('horizontal'))}
        disabled={selected.length < 3}
      />
      <Btn
        icon={<DistributeV className="w-3.5 h-3.5" />}
        tip="Distribute V"
        onClick={act(() => distributeLayers('vertical'))}
        disabled={selected.length < 3}
      />
    </div>
  );
