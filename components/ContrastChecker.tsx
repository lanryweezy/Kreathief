 ...
 */
export function checkContrast(foreground: string, background: string, fontSize = 16): ContrastResult {
 ...
 */
export function suggestAccessibleColor(background: string, preferredColor: string, minRatio = 4.5): string {
...
export const ContrastChecker: React.FC<ContrastCheckerProps> = ({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* Details */}
      {showDetails && <span className="text-xs text-gray-500">{result.ratio.toFixed(2)}:1</span>}

      {/* Warning */}
      {!result.isAccessible && <span className="text-xs text-red-400">⚠️ Low contrast</span>}
    </div>
  );
};
