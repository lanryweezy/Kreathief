...
export const DropdownMenu: React.FC<DropdownMenuProps> = ({ label, items, className = '' }) => {
    ...
    return (
        <div className={`relative ${className}`} ref={menuRef}>
            ...
            {isOpen && (
                <div role="menu" className="absolute top-full left-0 mt-1 w-56 bg-[#1e1e1e] border border-gray-700 rounded-lg shadow-2xl py-1 z-[100] animate-fadeIn">
                ...
                `}
                                >
                                    ...
                                    {item.shortcut && <span className="text-[10px] opacity-50 font-mono pointer-events-none">{item.shortcut}</span>}
                                </button>
                            )}
                        </React.Fragment>
                    ))}
