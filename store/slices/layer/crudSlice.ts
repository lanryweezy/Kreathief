...
export const createCRUDSlice: StateCreator<StoreState, [], [], Partial<LayerSlice>> = (set, get) => ({
  ...
  magicResize: (newWidth: number, newHeight: number, newName?: string) => {
    ...
    const newLayers = currentArtboard.layers.map((l: Layer) => {
      ...
      switch (constraints.horizontal) {
        case 'start': break;
        case 'end': { const rightDist = oldWidth - (lx + lw); lx = newWidth - lw - rightDist; break; }
        case 'center': { const centerX = lx + lw / 2; const relCenterX = centerX / oldWidth; lx = relCenterX * newWidth - lw / 2; break; }
        case 'both': { const leftDistBoth = lx; const rightDistBoth = oldWidth - (lx + lw); lx = leftDistBoth; lw = newWidth - leftDistBoth - rightDistBoth; break; }
        case 'scale':
        default: { const relX = lx / oldWidth; const relW = lw / oldWidth; lx = relX * newWidth; lw = relW * newWidth; break; }
      }

      switch (constraints.vertical) {
        case 'start': break;
        case 'end': { const bottomDist = oldHeight - (ly + lh); ly = newHeight - lh - bottomDist; break; }
        case 'center': { const centerY = ly + lh / 2; const relCenterY = centerY / oldHeight; ly = relCenterY * newHeight - lh / 2; break; }
        case 'both': { const topDistBoth = ly; const bottomDistBoth = oldHeight - (ly + lh); ly = topDistBoth; lh = newHeight - topDistBoth - bottomDistBoth; break; }
        case 'scale':
        default: { const relY = ly / oldHeight; const relH = lh / oldHeight; ly = relY * newHeight; lh = relH * newHeight; break; }
      }
    ...
    set((state: any) => ({
      artboards: [
        ...state.artboards,
        { id, name: newName || `${currentArtboard.name} (Resized)`, x, y: 0, width: newWidth, height: newHeight, layers: newLayers, backgroundColor: currentArtboard.backgroundColor },
      ],
      activeArtboardId: id,
      selectedLayerIds: [],
    }));
  ...
  addLayer: (layer) => {
    get().saveToHistory?.();
    set((state: any) => ({
      artboards: state.artboards.map((a: Artboard) => a.id === state.activeArtboardId ? { ...a, layers: [...a.layers, layer] } : a),
  ...
  addTextLayer: (style = {}) => {
    ...
    const newLayer: TextLayer = {
      ...
      x: artboard.width / 2 - width / 2, y: artboard.height / 2 - fontSize / 2,
      width: width, height: fontSize, rotation: 0,
      fontSize: fontSize, fontWeight: style.fontWeight || '700',
      fontFamily: style.fontFamily || 'Inter', fontStyle: style.fontStyle || 'normal',
      textDecoration: style.textDecoration || 'none', textAlign: style.textAlign || 'center',
      color: style.color || '#000000', opacity: style.opacity ?? 1,
      locked: false, visible: true,
      filters: { ...DEFAULT_LAYER_FILTERS }, blendMode: 'normal',
      skewX: 0, skewY: 0, perspective: 0, rotateX: 0, rotateY: 0,
      lineHeight: style.lineHeight || 1.2, letterSpacing: style.letterSpacing || 0,
      textTransform: style.textTransform || 'none',
      warpStyle: style.warpStyle || 'none', curve: style.curve || 0,
      ...style,
    };

    set((state: any) => ({
      artboards: state.artboards.map((a: Artboard) => a.id === state.activeArtboardId ? { ...a, layers: [...a.layers, newLayer] } : a),
  ...
  addImageLayer: (src, name = 'Image', x, y, width = 300, height = 300) => {
    ...
    const newLayer: ImageLayer = {
      id: generateLayerId('image'), type: 'image', name, src,
      x: x ?? artboard.width / 2 - width / 2, y: y ?? artboard.height / 2 - height / 2,
  ...
  addShapeLayer: (type, style = {}) => {
    ...
    const newLayer: ShapeLayer = {
      ...
      cornerRadius: DEFAULT_CORNER_RADIUS, opacity: 1, locked: false, visible: true,
      filters: { ...DEFAULT_LAYER_FILTERS }, blendMode: 'normal',
      skewX: 0, skewY: 0, perspective: 0, rotateX: 0, rotateY: 0,
      stroke: { color: '#94a3b8', width: 1 }, ...style,
    };
    set((state: any) => ({
      artboards: state.artboards.map((a: Artboard) => a.id === state.activeArtboardId ? { ...a, layers: [...a.layers, newLayer] } : a),
      selectedLayerIds: [newLayer.id],
  ...
  updateLayer: (id, partial) =>
    set((state: any) => {
      ...
      for (const a of state.artboards) {
        ...
        if (l?.componentId) { masterComponentId = l.componentId; break; }
      }
      ...
      return {
        artboards: state.artboards.map((a: Artboard) => ({
          ...
          layers: a.layers.map((l: Layer) => {
            if (l.id === id) {
              if (l.locked && sanitizedPartial.locked !== false && !('locked' in sanitizedPartial && Object.keys(sanitizedPartial).length === 1)) return l;
            ...
            if (masterComponentId && l.masterId === masterComponentId) {
              ...
              Object.keys(syncPartial).forEach((key) => { if (overrides.includes(key)) delete (syncPartial as any)[key]; });
  ...
  deleteLayer: (id) => {
    ...
    set((state: any) => ({
      artboards: state.artboards.map((a: Artboard) => ({
        ...a,
        layers: a.layers.filter((l: Layer) => l.id !== id).map((l: Layer) => {
          const cleaned = { ...l };
  ...
  pasteLayer: (style = {}) => {
    ...
    set((state: any) => ({
      artboards: state.artboards.map((a: Artboard) => a.id === activeArtboardId ? { ...a, layers: [...a.layers, newLayer] } : a),
      selectedLayerIds: [newLayer.id],
    }));
  },
  ...
  autoNameLayer: async (id: string) => {
    ...
    artboards.forEach((a: Artboard) => { const found = a.layers.find((l) => l.id === id); if (found) layer = found; });
    if (!layer) return;
