...
describe('pathLength', () => {
  it('straight line = euclidean distance', () => {
    expect(pathLength([{ points: [{ x: 0, y: 0 }, { x: 3, y: 4 }] }])).toBeCloseTo(5);
  });
  ...
  it('closed segment adds closing distance', () => {
    const seg: Segment = { points: [{ x: 0, y: 0 }, { x: 3, y: 0 }, { x: 3, y: 4 }], closed: true };
...
describe('bezierPathLength', () => {
  ...
  it('2 points → euclidean distance', () => {
    expect(bezierPathLength([{ x: 0, y: 0 }, { x: 3, y: 4 }])).toBeCloseTo(5, 0);
  });
...
describe('pathArea', () => {
  it('fewer than 3 points → 0', () => {
    expect(pathArea([{ x: 0, y: 0 }, { x: 1, y: 1 }])).toBe(0);
  });

  it('square area', () => {
    expect(pathArea([{ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 4, y: 4 }, { x: 0, y: 4 }])).toBeCloseTo(16);
  });

  it('triangle (shoelace)', () => {
    expect(pathArea([{ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 0, y: 3 }])).toBeCloseTo(6);
  });

  it('positive regardless of winding', () => {
    expect(pathArea([{ x: 0, y: 0 }, { x: 0, y: 4 }, { x: 4, y: 4 }, { x: 4, y: 0 }])).toBeCloseTo(16);
  });
});
...
describe('centroid', () => {
  ...
  it('symmetric shape → center', () => {
    const c = centroid([{ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 4, y: 4 }, { x: 0, y: 4 }]);
