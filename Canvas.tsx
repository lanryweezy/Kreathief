...
const CanvasComponent: React.FC<CanvasProps> = (props) => {
  ...
  const allLayers = useMemo(() => {
    ...
    const prev = allLayersRef.current;
    ...
    if (prevIds === newIds) {
      // Same IDs — also verify layer count and do shallow content comparison
      if (prev.length === newLayers.length) {
        let contentChanged = false;
        for (let i = 0; i < prev.length; i++) {
          if (prev[i] !== newLayers[i]) {
            contentChanged = true;
            break;
          }
        }
  ...
  const viewportBounds = useMemo(() => {
    ...
    const prev = viewportBoundsRef.current;
