...
export const templateMarketplace = {
    ...
    try {
      ...
      const { data: row, error } = await supabase
        ...
        .insert({
          ...
          author_name: user.user.user_metadata?.full_name || user.user.email || 'Anonymous',
          author_avatar: user.user.user_metadata?.avatar_url,
          status: 'pending',
        })
        .select()
        .single();
      ...
      log.error('[TemplateMarketplace] Submit failed', e);
      return null;
    }
  },
  ...
  async getTemplates(
    category?: string,
    page = 1,
    sortBy: 'popular' | 'recent' | 'likes' = 'popular'
    ...
    try {
      ...
      log.error('[TemplateMarketplace] Fetch failed', e);
  ...
  async getTemplateById(id: string): Promise<MarketplaceTemplate | null> {
    try {
      ...
      log.error('[TemplateMarketplace] Get by ID failed', e);
      return null;
    }
  },
  ...
  async getPopularTemplates(limit = 8): Promise<MarketplaceTemplate[]> {
    try {
      ...
      log.error('[TemplateMarketplace] Popular fetch failed', e);
      return [];
    }
  },
  ...
  async getRecentTemplates(limit = 8): Promise<MarketplaceTemplate[]> {
    try {
      ...
      log.error('[TemplateMarketplace] Recent fetch failed', e);
      return [];
    }
  },
  ...
  async searchTemplates(query: string, limit = 20): Promise<MarketplaceTemplate[]> {
    try {
      ...
      log.error('[TemplateMarketplace] Search failed', e);
      return [];
    }
  },
  ...
  async getTemplatesByUser(userId: string): Promise<MarketplaceTemplate[]> {
    try {
      ...
      log.error('[TemplateMarketplace] User fetch failed', e);
      return [];
    }
  },
};
