...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const cssRes = await fetch('https://fonts.googleapis.com/css2?family=Material+Icons', { headers: { 'User-Agent': 'Mozilla/5.0' } });
    ...
    return new Response(JSON.stringify({ icons: filtered }), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': origin!, ...cacheHeaders(86400) } });
  } catch (err: any) {
